 ...
 * @date 2022-11-01
 * @brief EHP DashBoard - e-HC 평균 점수 컴포넌트
 *
 ********************************************************************/
...
import SpgCheckList from "../../jesttest/management/checksheet/SpgCheckListTest";
 ...
 /**
 * @brief EHP DashBoard - e-HC 평균 점수 컴포넌트, 반응형 동작
 * @param -
...
const LCONST_NUM_CARDWSIZE = 450;
function EhcAverageScore(props) {
  //trans, ref, navigate
  ...
  useEffect(() => { // resize handler
    ...
    function handleResize() {
      ...
      if (!CUTIL.isnull(mobileTag)) {
        //clog("XX WIDTH : " + mobileTag.clientWidth);
        if ((mobileTag.clientWidth <= LCONST_NUM_CARDWSIZE)) {
          setIsMobile(true);
        } else {
          setIsMobile(false);
        }
  ...
  useDebounceEffect(
    async () => {
      if (mobileRef.current) {
        //clog("XX WIDTH : " + mobileRef.current.clientWidth);
        if ((mobileRef.current.clientWidth <= LCONST_NUM_CARDWSIZE)) {
          setIsMobile(true);
        }
      }
    }, 100, [ehcAverage],
  ...
  function handleChartVal(val) {
    ...
    setChartVal({ 
      ...chartVal, 
      radar:{...chartVal.radar, 
        indicator:indicators, 
        axisName : {fontWeight: 500, fontFamily: "Noto Sans KR", fontSize: (isMobile)?12:16}
      },
  ...
  return (
  <>
  <div className="box__header" ref={mobileRef}>
  ...
  ?<div className="box__body">
    <div className="top mt-20">
      <div className={`ehc__info ehc-${(step.stepVal==="ADVANCED")?"a":(step.stepVal==="PREMIUM")?"p":"b"}`}>
        <p className="ehc__name">{selTree.company.companyName} {(selTree.zone.zoneId.length > 0)?selTree.zone.zoneName:""}</p>
      </div>
    </div>
...
export default EhcAverageScore;
