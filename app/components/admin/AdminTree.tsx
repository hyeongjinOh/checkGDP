...
import { loadingBoxState, czoneInfoState } from "../../recoil/menuState";
...
/**
 * @brief EHP 운영 관리 관리자 Tree 컴포넌트 , 반응형 동작
 * @param -
...
function ZoneTree(props) {
  ...
  function onClickZone(e, zone) {
    ...
    e.stopPropagation();
    //clog("IN ADMINTREE : onClickZone : " + e.target.tagName + " : " + e.target.getAttribute("data-val"));
    ...
    <>
      {(zoneTree) && zoneTree.map((zone, idx) => (
        (zone.approval === 2)
          ...
          : (zone.approval === 1)
            ? <li key={`zone_${idx.toString()}`} className="add__wait" id={`li_focus_${zone.zoneId}`}>
              <span className="name">
                <span>{props.company.companyName}</span>
                <span>{zone.zoneName}</span>
              </span>
