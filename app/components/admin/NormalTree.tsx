...
/**
 * @brief EHP 운영 관리 사용자 Tree 컴포넌트 , 반응형 동작
 * @param -
...
function NormalTree(props) {
  ...
  const { data: resInfo, error, isLoading, reload, run, } = useAsync({
    ...
    watch: adminType + workMode + curTree.reload
  });
  ...
  useEffect(() => {
    ...
    if (resInfo/* !isLoading */) {
      ...
      if (resInfo.codeNum == CONST.API_200) {
        ...
        setParentCurTree(adminType, { ...curTree, reload: false });
      }
    }
  ...
  function onClickZoneList(e) {
    ...
    } else { // ADMIN 경우, 사업장 추가 기능이 없어짐 => 검색영역으로 바뀜
      setParentWorkMode("LIST");
  ...
  return (
    <>
      ...
      <div className="area__left">
        {(tabType === "TAB_ZONE") && <div className="lnb__top">
          ...
          <button type="button"
            ...
            //data-pop="pop-area-right-page-info" // insert 후 뷰 이동 위해
...
function ZoneTree(props) {
  ...
  function onClickZone(e, zone) {
    setParentPopWin("pop-area-right-page-info", null);
    e.stopPropagation();
    //clog("IN NORMAL TREE : onClickZone : " + e.target.tagName + " : " + e.target.getAttribute("data-val"));
    ...
    <>
      {(subZoneTree) && subZoneTree.map((subZone, idx) => (
        ...
        >
          ...
          <ul className={`lnb-depth3 ${(tabType == "TAB_DEVICE") ? "no-btn" : ""} `}
  ...
  return (
    <>
      ...
      <div className="popup__footer">
        ...
        <button type="button" /* className="btn-close" */>
          <span onClick={(e) => CUTIL.jsclose_Popup("pop-reason")}>확인</span>
        </button>
      </div>
    </>
  )
