...
/**
 * @brief EHP 사업장 전기실 관리 - 전기실 수정 컴포넌트 , 반응형 동작
 * @param -
...
function RoomUpdate(props) {
          ...
          if ( savedFiles.length <= 0 ) {
            ...
            savedFiles.map((file, idx) => {
              if (file.hasOwnProperty("type") && (file.type == "INS")) {
                ...
                deleteAttachFiles(data.body, file.imageId, uploadFileHandler);
              }
            });
          }
        })();
        /*
        setParentCurTree("ROOM",{...curTree, room: { "roomId": data.body.roomId, "roomName": data.body.roomName },});
        setParentWorkMode("READ");
        */
      } else { // api if
        // need error handle
  ...
  async function saveAttachFile(roomInfo, fileFormData, handlerFunc) {
    ...
    if (data) {
      //clog("saveFiles : CHECK : " + JSON.stringify(data));
  ...
  async function deleteAttachFiles(roomInfo, imgId, handlerFunc) {
    let data: any = null;
    data = await HTTPUTIL.PromiseHttp({
      "httpMethod": "DELETE",
            ...
            <div className="content__info h-auto">
              ...
              <ul className="form__input">
                {/* <li>
                  ...
                  <p className={`txt ${(roomInfo.isontree) ? "treeon" : ""} mt-5`}>
