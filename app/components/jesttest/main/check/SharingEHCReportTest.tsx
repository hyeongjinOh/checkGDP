...
function EhcCheckResult(props) {
    ...
    function handleResize() {
        /*         clog("IN BASIC : MOBILE CHECK : RESIZE : clientHeight :" + mobileTag.clientHeight);
                  clog("IN BASIC : MOBILE CHECK : RESIZE : clientWidth :" +window.innerWidth);
                clog("IN BASIC : MOBILE CHECK : RESIZE :" + isMobile);  */
        if (CUTIL.isnull(mobileTag)) return;
        if ((mobileTag.clientHeight <= 787) && (window.innerWidth <= 500)) {
            // 모바일 셋팅
            if (!isMobile) return setIsMobile(true);
        } else {

            if (isMobile) return setIsMobile(false);
        }
    }
    ...
    function hadleRerun(val) {
        ...
        //     assessment: {
        //         //preAssessmentId: curItem.assessment.preAssessmentId,
        //         assessmentId: null,
        //         totalComment: null,
        //         reportId: null,
        //         updatedTime: null,
    ...
    async function onSharePop(e) {
        ...
        if (data) {
            if (data.codeNum == CONST.API_200) {
                //const fileLink = `${window.location.origin}${CONST.URL_COMM_PDFVIEWER}?${CONST.STR_PARAM_DATA + CONST.STR_PARAM_PDFNM}=${curItem.itemName}_진단점검리포트.PDF&${CONST.STR_PARAM_DATA + CONST.STR_PARAM_PDFID}=${data.body.fileLink}`
                setShareOn(true);
                //setLink(fileLink)
            } else {
        ...
        <>
            ...
            <div className="tbl__bottom">
                <ul className="btn__wrap">
                    ...
                    < li >
                        {/* <!--220616, data-pop추가 --> */}
                        {(alarmDto != null) && (alarmDto.alarmStatus != "SAFETY") &&
                            < button type="button" className={`btn-checkpop  "bg-green" "bg-yellow" : ""}`} data-pop="pop-check" onClick={(e) => onClickPremiumOpen(e)}><span>점검 요청</span></button>
            ...
            < div id="pop-reload" className="popup-boxin" >
                <div className="page-detail w400">
                    ...
                    <div className="popup__body">
                        <p className="fontMedium">점검을 재진행하시겠습니까?</p>
                        <p>이전 점검 내용은 사라지게 됩니다.</p>
                    </div>
                    <div className="popup__footer">
                        <button type="button" className="bg-gray" onClick={(e) => rerunPopClose(e)} ><span>취소</span></button>
                        <button type="button" className="close" onClick={(e) => hadleRerun(e)}><span>확인</span></button>
                    </div>
                </div>
            </div >
            {/* <!-- //재진행 팝업 --> */}
            ...
            {/* <!-- //수정 팝업 --> */}
            {(totalScore > 70) &&
                <NormarPop totalScore={totalScore} />
            }
            {/* <!--진단결과 공유 팝업 221011 --> */}
                ...
                <div className="page-detail">
                   ...
                   <p className="input-errortxt">올바르지 않은 이메일 주소입니다.</p> */}
                    ...
                    <div className="popup__footer">
                        ...
                        <button type="button" className="close"><span>확인1</span></button>
                    </div>
                </div>
            </div>
            {/* <!-- //진단결과 공유 팝업 --> */}
...
function NormarPop(props) {
    ...
    return (
        <>
            {/* <!--70점 이상 시 노말 팝업 --> */}
            <div id="pop-check" className={`popup-boxin ${(totalScore > 70) ? "on" : ""}`}>
                <div className="page-detail w400">
                    <div className="popup__head">
                        <h1>진단점검 결과</h1>
                        <button className="btn btn-close " onClick={(e) => rerunPopClose(e)}><span className="hide">닫기</span></button>
                    ...
                    <div className="popup__body">
                        <p className="fontMedium">진단점검 결과 70점 이상으로 정상(NORMAL) 단계로 이동합니다.</p>
                        {/* <p>(출장 및 현장 정밀 진단 서비스 비용 발생)</p> */}
                    </div>
                    <div className="popup__footer">
                        <button type="button" className="close js-close" onClick={(e) => rerunPopClose(e)}><span>확인</span></button>
                    </div>
                </div>
            </div>
            {/* <!-- 70점 이상 시 노말 팝업 --> */}
