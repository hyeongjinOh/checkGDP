         ...
         <div className="tbl-list hcal-vh-315">
           <table summary="기기 명, 모델 명, 담당자, 승인요청 항목으로 구성된 진단점검 승인 요청 리스트 입니다.">
             ...
             <tbody>
               {checkList.map((list, idx) => (
                 <tr key={"CHECK_" + idx}>
                   ...
                   <td className="txt-left">
                     <img src={(list.itemStep == "BASIC_DONE" && list.oldDeviceConsult == 0) ?
                       require("/static/img/icon_p.png")
                       :
                       require("/static/img/icon_c.png")
                     }
                       style={{ "width": "24px", "height": "24px" }}
                       alt={(list.itemStep == "BASIC_DONE" && list.oldDeviceConsult == 0) ?
                         "Premium" : "change"}
                     />
                   </td>
                   <td className="txt-left">{list.spg.name}</td>
                   <td className="txt-left"><p className="ellipsis">{list.itemName}</p></td>
                   <td className="txt-left">{list.responsible}</td>
                   <td>
                     {(list.itemStatus == "REQUESTED") ?
                       <button type="button" className="bg-blue center js-open" data-pop="pop-casecheck-ok" onClick={(e) => checkPop(e, list)}><span data-pop="pop-casecheck-ok">요청 완료</span></button>
                       : (list.oldDeviceConsult == 1) ?
                         <button type="button" className="bg-pink center js-open" data-pop="pop-casecheck-ok" onClick={(e) => checkPop(e, list)}><span data-pop="pop-casecheck-ok">교체요청</span></button>
                         :
               ...
               {/*         {checkList.filter((sort) => (sort.itemStatus == "ACCEPTED")).map((list, idx) => (
                  <tr key={"CHECK" + idx}>
 ...
 function CheckApprovalPop(props) {
   ...
   return (
     <>
       ...
       {(itemDetailInfo) && <div className="popup__body layout-vertical">
         <h2>
         ...
         <div className="tbl-list type2">
           <table summary="사용자 타입, 회사, 이름, E-mail(ID), 연락처 항목으로 구성된 Engineer 정보 목록 입니다.">
             ...
             <tbody>
               {(engName.length > 0)
                 ? (engineerList.filter(eng => (eng.userName.toUpperCase().includes(engName.toUpperCase()))).length <= 0)
                   ?
                   <tr key={`en_tr_no`}>
                     <td colSpan={7}><p className="nodata-intable">검색된 Engineer가 없습니다.</p></td>
                   </tr>
                   : engineerList.filter(eng => (eng.userName.toUpperCase().includes(engName.toUpperCase()))).map((eng, idx) => (
                     <tr key={`en_tr_${idx.toString()}`}>
                       <td className="txt-left fontMedium">{(eng.role == "ENGINEER") ? "Engineer" : ""}</td>
                       <td className="txt-left">{eng.companyName}</td>
                       <td className="txt-left">{eng.userName}</td>
                       <td className="txt-left">{eng.email}</td>
                       <td className="txt-left">{eng.phoneNumber}</td>
                       <td className="txt-left">{eng.itemCnt}</td>
                       <td>
                         {(checkChange != eng.userIdPk) &&
                           <button type="button" className="btn-basic" onClick={(e) => engineerSelect(e, eng)}><span>선택</span></button>
                         }
                         {(checkChange == eng.userIdPk) &&
                 ...
                 : engineerList.map((eng, idx) => (
                   <tr key={`en_tr_${idx.toString()}`}>
                     ...
                     <td>
                       {(checkChange != eng.userIdPk) &&
                         <button type="button" className={(checkChange != eng.userIdPk) ? "btn-basic" : "btn-basic bg-navy"} onClick={(e) => engineerSelect(e, eng)}><span>선택</span></button>
                       }
                       {(checkChange == eng.userIdPk) &&
                         <button type="button" className="btn-basic bg-navy" onClick={(e) => engineerSelect(e, eng)} ><span>배정</span></button>
                       }
