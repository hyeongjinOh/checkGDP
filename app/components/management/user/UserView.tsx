...
export default function UserView(props) {
  ...
  //
  let appPath = "companyName=" + selTree.company.companyName + '&page=' + pageInfo.number + '&size=' + pageInfo.size;
  ...
  const { data: data, isLoading } = useAsync({
    ...
    watch: appPath + selTree.reload + reload + enableItem + userLoad + load
  });
  ...
  useEffect(() => {
    ...
    if (data) {
      if (data.codeNum == CONST.API_200) {
        ...
        if (data.errorList.filter((err) => (err.field == "companyName"))) {
  ...
  useEffect(() => {
    setParentPopWin("pop-userjoin-info",
      <SiteCeatePop
        htmlHeader={(item) && <h1>{item.userName}</h1>}
        item={item}
  ...
  // <!-- 신규 가입자 - enablde 팝업  --> 
  useEffect(() => {
    setParentPopWin("pop-enabled",
      <EnabledPop
        htmlHeader={(item) && <h1>사용자 설정</h1>}
        htmlHeaderBtn={(item) && <button className="btn btn-close js-close" onClick={(e) => closeBtn("pop-enabled")} ><span className="hide">닫기</span></button>}
  ...
  // <!-- 신규 가입자 - password 팝업  --> 
  useEffect(() => {
    setParentPopWin("pop-password",
      <PassswordPop
        htmlHeader={(item) && <h1>패스워드 초기화</h1>}
        htmlHeaderBtn={(item) && <button className="btn btn-close js-close" onClick={(e) => closeBtn("pop-password")} ><span className="hide">닫기</span></button>}
  ...
  useEffect(() => {
    setParentPopWin("pop-delete",
      <DeletePop
        htmlHeader={(item) && <h1>회사_사업장 목록</h1>}
        htmlHeaderBtn={(item) && <button className="btn btn-close js-close" onClick={(e) => closeBtn("pop-delete")} ><span className="hide">닫기</span></button>}
    ...
    <>
      ...
      <div className="area__right_content workplace__info workplace__main info__input newtype">
        <div className="page-top">
          <h2>{selTree.company.companyName}</h2>
        </div>
    ...
    if (data) {
      if (data.codeNum == CONST.API_200) {
        ...
        if (data.errorList.filter((err) => (err.field == "userId"))) {
          ...
          CUTIL.jsclose_Popup("pop-userjoin-ok");
        }
      ...
      <li className="inline">
        ...
        {(userType == roleDto[3].value) && (list.enabled == true) ?
          <button type="button" className="btn-basic js-open d-sm-none" data-pop="pop-userjoin-info"
            onClick={(e) => { CUTIL.jsopen_Popup(e), companySelect(e) }}>
    ...
    <>
      {(item) &&
        <div className="popup__body">
          <p>해당 사용자를 {(enableType == true) ? "비활성화" : "활성화"}하시겠습니까?</p>
...
function PassswordPop(props) {
  ...
  return (
    <>
      {(item) &&
        <div className="popup__body">
          <p >해당 사용자 패스워드가 초기화 {(!passwordItem) ? "하시겠습니까?" : "되었습니다."}</p>
