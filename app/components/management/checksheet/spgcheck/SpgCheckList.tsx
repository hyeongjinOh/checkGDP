...
/**
 * @brief EHP Check Sheet - List 컴포넌트, 반응형 동작
 * @param -
...
function SpgCheckList(props) {
  ...
  function onClickSpgVersionChange(e, sver) {
    var isChange = false;
    versionList.filter(sv=>sv.enabled).map((sv, idx)=>{
      if ( (idx === 0) && (sv.versionNo !== sver.versionNo) ) isChange = true;
    });
    if ( isChange === false) {
      return;
    }
    ...
    if (CUTIL.isnull(popupVal)) e.target.setAttribute("data-ds-height", "267");
  ...
  clog("IN SPG CHECK LIST TEMP : INIT : " + JSON.stringify(spgVersion) + "" + JSON.stringify(versionList));
    ...
    <div className="area__right_content workplace__info workplace__main info__input newtype">
      ...
      <div className="tbl__top mb-16">
        <div className="left">
          <div className="searcharea">
            <div className="searchinput">
              ...
              {(!spgVersion)&&<div className="select w80" onClick={(e)=>CUTIL.onClickSelect(e, selectOptionVersion)}>
                <div className="selected">
                  <div className="selected-value">x1.0</div>
                  <div className="arrow"></div>
                </div>
                <ul>
                    <li key={`ver_x`}
              ...
              {(spgVersion)&&(versionList)&&
              <div className="select w80" onClick={(e)=>CUTIL.onClickSelect(e, selectOptionVersion)}>
                <div className="selected">
          ...
          <button type="button"
            className={`${(spgVersion&&versionList)&&versionList.filter(sv=>sv.enabled).map(sv=>(sv.versionNo!==spgVersion.versionNo)?"bg-basic":"bg-gray")} btn-basic w96 ml-16 js-open`}
        ...
        <div className="right">
          <button type="button" className="bg-gray js-open" data-pop="pop-version"
            onClick={(e)=>onClickSpgVersionAdd(e)}
          >
            <span>버전 수정/추가</span>
          </button>
        </div>
      </div>
      {(spgVersion)&&/*(checkList.length > 0)&&*/
        <CheckList
          adminType={props.adminType}
          selTree={selTree}
          selStep={selStep}
...
function CheckList(props) {
  ...
  function onClickViewSpgCheck(e, checkInfo) {
    ...
    popupVal = e.target.getAttribute("data-ds-height");
    if (CUTIL.isnull(popupVal)) e.target.setAttribute("data-ds-height", "797");
...
function PopupSpgCheckView(props) {
  ...
  clog("SPG CHECK VIEW : INIT : " + clistMode + " : " + updateCount);
  const { data: retData, error, isLoading, reload, run, } = useAsync({
    promiseFn: HTTPUTIL.PromiseHttp,
    httpMethod: "GET",
    ...
    userToken: userInfo.loginInfo.token, // url 직접 접근시 로그인 정보 사라짐..
    watch: checkInfo.code+clistMode+updateCount
  });
  useEffect(() => {
    setRecoilIsLoadingBox(true);
    if (retData) {
