  ...
  return (
    ...
    <div className="area__right_content workplace__info workplace__main info__input newtype">
      <div className="page-top">
        <h2>{selTree.spg.spgName} {(spgNewVersion)&&spgNewVersion.versionNo}
          <span className="txt-info">
            <span className="point">수정 중</span> *우측 [작업완료] 버튼을 클릭하여 수정/추가 작업을 완료해주세요. 
          </span>
        </h2>
      </div>
...
function CheckList(props) {
  ...
  async function onClickDeleteSpgCheck(e, checkInfo) {
    var isOk = confirm(`해당 항목를 삭제하시겠습니까?`);
    if ( !isOk ) return;
    let data: any = null;
    setRecoilIsLoadingBox(true);
      //CUTIL.sleep(500);
  ...
  clog("SPG CHECK LIST : INIT : " + workMode);
  return (
    <>
      {/*<!--테이블-->*/}
      <div className="tbl-list checksheet-list">
        <table summary="점검단계,코드,그룹,이름,설명,가중치 항목으로 구성된 ACB 리스트 입니다.">
          ...
          <tbody>
            {(!spgNewVersion)&&
            <tr>
              <td colSpan={8} className=""><strong className="txt-purple">Version을 선택하세요.</strong></td>  
            </tr>
            }
            ...
            <tr key={`sh_${idx.toString()}`}>
              ...
              <td>
                <button type="button" className="btn btn-delete-gr js-open" data-pop="pop-delete"
                  onClick={(e)=>onClickDeleteSpgCheck(e, check)}
                >
...
function PopupSpgCheckView(props) {
  ...
  const { data: retData, error, isLoading, reload, run, } = useAsync({
    ...
    userToken: userInfo.loginInfo.token, // url 직접 접근시 로그인 정보 사라짐..
    watch: checkInfo.code+workMode+updateCount
  });
  useEffect(() => {
    setRecoilIsLoadingBox(true);
    if (retData) {
