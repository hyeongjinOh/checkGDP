...
/**
 * @brief EHP 이메일 상담 관리 - List 컴포넌트, 반응형 동작
 * @param -
...
function EconsultList(props) {
  ...
  */
  ...
  function onClickAnswerOk(e, consult) {
    ...
    if (CUTIL.isnull(popupVal)) e.target.setAttribute("data-ds-height", "730");
  ...
  const [winCountReply, setWinCountReply] = useState(0);
  function onClickAnswer(e, consult) {
    ...
    setParentPopWin("pop-answer",
      <MobileEmailConsultAnswer
        ...
        winCountReply={winCountReply + 1}
        setWinCountReply={setWinCountReply}
        setPopWin={props.setPopWin}
      />
    );
    CUTIL.jsopen_Popup(e);
    setWinCountReply(winCountReply + 1);
  }
  ...
  return (
    <>
      ...
      <div className="area__right w100p">
        ...
        <div className="tbl-list checksheet-list hcal-270 ">
...
function MobileEmailConsultAnswer(props) {
  ...
  const winCountReply = props.winCountReply;
  const setParentWinCountReply = props.setWinCountReply;
  const setParentPopWin = props.setPopWin;
  ...
  clog("ERROR LIST : " + JSON.stringify(errorList));
          ...
          <ul className="form__input mb-0">
            <li className="column">
              ...
              <div className="input__area mt-0">
                <textarea className={`h128 line-h22 ${(errorList.filter(err => (err.field === "reply")).length > 0) ? "input-error" : ""}`}
                  value={reply}
                  onKeyPress={(e) => CUTIL.beforeHandleComment(e, 1000)}
                  onKeyUp={(e) => CUTIL.afterHandleComment(e, 1000, handleSetReply)}
                  onChange={(e) => handleSetReply(e)}
                />
                <p className="input-errortxt">{errorList.filter(err => (err.field === "reply")).map((err) => err.msg)}</p>
                <p className="txtnum inline left">* 첨부파일은 10개 이하, 전체 20M까지 가능합니다.</p>
              </div>
            </li>
            <li className="sub-input mt-8">
              ...
              <div className="input__area">
                <div className="filebox">
                  <input className="upload-name" placeholder="파일을 첨부하세요" readOnly />
