        ...
        <>
            <div className="area__right">
                <ul className="page-loca">
                    ...
                    {(selTree.subZone.subZoneId.length > 0) &&
                        <li>{selTree.subZone.subZoneName}</li>}
                </ul>
                {/*<!--검색영역-->*/}
                            ...
                            <li>
                                <div className={(serviceStatus === "") ? "on" : ""}>
                                    <a href="#" onClick={(e) => handleServiceStatus("")}>
                                        <p>{t("LABEL.전체")}</p>
                                        <p><strong>{changeCount.totalCount}</strong>ea</p>
                                    </a>
                                </div>
                            </li>
                            <li>
                                <div className={(serviceStatus === "request") ? "on" : ""}>
                                    <a href="#" onClick={(e) => handleServiceStatus("request")}>
                                        <p>{t("LABEL.요청")}</p>
                                        <p><strong>{changeCount.requestCount}</strong>ea</p>
                                    </a>
                                </div>
                                <div className={(serviceStatus === "accept") ? "on" : ""}>
                                    <a href="#" onClick={(e) => handleServiceStatus("accept")}>
                                        <p>{t("LABEL.접수")}</p>
                                        <p><strong>{changeCount.acceptCount}</strong>ea</p>
                                    </a>
                                </div>
                                {/*    <div className={(serviceStatus === "ING") ? "on" : ""}>
                                    <a href="#" onClick={(e) => handleServiceStatus("ING")}>
                                        <p>{t("LABEL.진행중")}</p>
                                        <p><strong>{changeCount.ingCount}</strong>ea</p>
                                    </a>
                                </div>
                                <div className={(serviceStatus === "DONE") ? "on" : ""}>
                                    <a href="#" onClick={(e) => handleServiceStatus("DONE")}>
                                        <p>{t("LABEL.완료")}</p>
                                        <p><strong>{changeCount.doneCount}</strong>ea</p>
                                    </a>
                                </div> */}
                            </li>
                        </ul>
...
function ChangeDetailList(props) {
    //trans
    const t = useTrans();
    const navigate = useNavigate();
    const mobileRef = useRef(null); // Mobile Check용
    ...
    const setParentPopWin = props.setPopWin;
    const setIsCount = props.setIsCount;
    //
    ...
    const { data: retData, error, isLoading, reload, run, } = useAsync({
        promiseFn: HTTPUTIL.PromiseHttp,
        httpMethod: "GET",
        "appPath": `/api/v2/product/company/zone/subzone/room/panel/item/devicechange?${listPath}`,
        appQuery: {},
        userToken: userInfo.loginInfo.token, // url 직접 접근시 로그인 정보 사라짐..
    ...
    function onClickEhcDetailView(e, ehc) {
        ...
        setParentPopWin("pop-list-view",
            <MobileEhcDetailView
                htmlHeader={<h1>노후교체 {t("LABEL.상세정보")}</h1>}
                list={ehc}
                setPopWin={props.setPopWin}
            />
        );
            ...
            <div className="tbl-list hcal-vh-560">
                <table summary="상태,점검단계,요청 일자,접수번호, SPG 명,Panel 명,시리얼 번호,담당자,점검일자,Step,Report,진행 일자,완료 일자 항목으로 구성된 e-HC Work Order List 입니다.">
                    ...
                    <thead>
                        <tr>
                            <th scope="col"
                                className={`${fieldList.filter(fd => fd.fdVal === "status").map(fd => (fd.sort) ? `sort ${fd.sort}` : "")} txt-left d-sm-none`}
                            ...
                            <th scope="col"
                                className={`${fieldList.filter(fd => fd.fdVal === "itemName").map(fd => (fd.sort) ? `sort ${fd.sort}` : "")} d-sm-none txt-left`}
                            ...
                            <th scope="col"
                                className={`${fieldList.filter(fd => fd.fdVal === "serialNo").map(fd => (fd.sort) ? `sort ${fd.sort}` : "")} d-lm-none d-sm-none txt-left`}
                    ...
                    <tbody>
                        {changeList.map((list, idx) => (
                            <tr key={"change_" + idx.toString()} className="js-open-m2" data-pop="pop-list-view" onClick={(e) => onClickEhcDetailView(e, list)}>
                                <td className="txt-left d-sm-none">{list.status}</td>
                                <td className="txt-left d-sm-none">{(list.requestDate) ? CUTIL.utc2formedstr(list.requestDate, "YYYY-MM-DD") : "-"}</td>
                                <td className="txt-left">{list.spg.spgName}</td>
                                <td className="txt-left"><p className="ellipsis">{list.panel.panelName}</p></td>
                                <td className="txt-left d-sm-none"><p className="ellipsis">{list.modelName}</p></td>
                                <td className="txt-left d-lm-none d-sm-none"><p className="ellipsis">{(list.serialNo) ? list.serialNo : "-"}</p></td>
                                <td className="txt-left d-lm-none d-sm-none">{(list.handlingDate) ? CUTIL.utc2formedstr(list.handlingDate, "YYYY-MM-DD") : "-"}</td>
                                <td className="txt-left d-lm-none d-sm-none">{(list.completionDate) ? CUTIL.utc2formedstr(list.completionDate, "YYYY-MM-DD") : "-"}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
