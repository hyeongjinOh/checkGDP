...
function ItemReportView(props) {
  ...
  useEffect(() => {
    if (onloadRef) {
      clog("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
      initTrRendering();
      /*
      var trClicks = document.querySelectorAll(".tr__click");
      for (var i = 0; i < trClicks.length; i++) {
  ...
  function pdfDownloadAll(e, reportList) {
    ...
    let zips = new JSZip();
    let zipCnt = 0;
    // const zipFileName = "검사성적서.zip" // 최효일PM님 지시로 문구 변경 by hjo-22011014
    ...
    reportList.map((report) => {
      ...
      const fileName = report.PDF_LINK.substring(sPos + 1, ePos);
      const objUrl = new URL(report.PDF_LINK);
      let reportLink = report.PDF_LINK;
      // reportLink = "https://tbs1.ls-electric.com:8443"+objUrl.pathname;
      clog("URL OBJ : " + objUrl.origin + " : " + objUrl.pathname);
      clog("IN ItemReports : pdfDownloadAll : " + zipCnt + " : " + fileName + " : " + reportLink);
          ...
          <section className="swiper-section" style={{ "cursor": "pointer" }}>
            <div className="swiper-container mySwiper">
              <div style={{ "cursor": "pointer" }} className="swiper-wrapper">
                ...
                {zoneList.filter((tzone, idx) => ((idx < zoneList.length)) && (idx >= tabPos)).map((tzone, idx) => (
                  <div
                    ref={tabRef}
                    key={tzone.zoneid + "_" + idx}
                    className={`swiper-slide tab ${(tzone.zoneId == curZone.zoneId) ? "on" : ""}`}
                    data-value={JSON.stringify(tzone)}
                    onClick={(e) => (tzone.zoneId !== curZone.zoneId) && onClickTab(e)}>
          ...
          <div className="d-sm-tab">
            <div className="select" onClick={(e) => CUTIL.onClickSelect(e, selectTabOption)}>
              <div className="selected">
                <div className="selected-value">{(curZone) && curZone.company.companyName + " " + curZone.zoneName}</div>
          ...
          <div id="tab-1" className="tabcontent current">
            ...
            <div className="tabcontent__top">
              ...
              <div className="search__inline">
                ...
                {/* <!--221110, 오른쪽 영역 감싸는 태그 (inline right) 생성--> */}
                <div className="inline right">
                  ...
                  {/* <!--221110, 시리얼번호 버튼 추가--> */}
                  <button type="button" className="btn btn-qr btn-basic mr-10 js-open" data-pop="pop-qr-web"
                    onClick={(e) => CUTIL.jsopen_Popup(e)}>
                    <span data-pop="pop-qr-web">{t("FIELD.시리얼번호")}</span>
                  </button>
                </div>{/*.search__inline*/}
              </div>{/* <!--//tabcontent__top--> */}
            </div>{/*.inline right  */}
            {(curZone) && <ItemReportTabCont
              //listInit={listInit}
              itemList={itemList}
  ...
  */
  ...
  useEffect(() => {
    if (retInit && (retInit.codeNum === CONST.API_200)) {
      ...
      setParentNodata(retInit.data.page);
  ...
  function onClickItemTr(e, ord, item) {
    ...
    if (!item.serialNo) { // serialNo가 null 값 API X
      setParentItemList(
          ...
          <tbody>
            ...
            {itemList.map((item, idx) => (
              <React.Fragment key={"frm_" + idx}>
                ...
                <tr key={`tr_${idx.toString()}_${searchTextField.searchText}`}
              ...
              <tbody>
                ...
                {(reportList) && reportList.map((report, idx) => (
                  <tr key={"report_" + idx}>
                    ...
                    <td className="d-sm-none">{report.TYPE} </td>
...
function QrCodeSearchPop(props) {
  ...
  const typePop = props.typePop;
  const setTypePop = props.setTypePop;
    ...
    <>
      ...
      <div id="pop-qr-scan" className="popup-layer js-layer layer-out hidden page-detail page-report" >
        <div className="popup__head">
          ...
          <button className="btn btn-close js-close" onClick={(e) => closeQrPop(e)}  ><span className="hide">닫기</span></button>
        </div>
        {(qrSerial) &&
          <div className="popup__body pb-0">
              ...
              <ul className="select-qr mt-20">
                ...
                {qrSerial.filter((txt, idx) => (idx == 0)).map((qrc, idx) => (
                  <li key={"qr_" + idx.toString()}>
                    <a href="#" className={(serialD === qrc[0]) ? "on" : ""} onClick={(e) => qrClick(qrc)}>{qrc}</a>
                  </li>
                ))}
                {(qrSerial) && qrSerial.filter((ftxt, idx) => (idx == 1)).map((qrc, idx) => (
                  <li key={"qr__" + idx.toString()}>
                    <a href="#" className={(serialD === qrc[idx] + qrc[idx + 1]) ? "on" : ""} onClick={(e) => qrClick(qrc[0] + qrc[1])}>
                      {qrc[idx] + " " + qrc[idx + 1]}</a>
                  </li>
                ))}
                {(qrSerial) && qrSerial.filter((ftxt, idx) => (idx == 2) && (ftxt[0])).map((qrc, idx) => (
                  <li key={"qr__" + idx.toString()}>
                    <a href="#" className={(serialD === qrc[0]) ? "on" : ""} onClick={(e) => qrClick(qrc[0])}>{
                      qrc[0]
                    }</a>
                  </li>
                ))}
                {(qrSerial) && qrSerial.filter((ftxt, idx) => (idx == 2) && (ftxt[1])).map((qrc, idx) => (
                  <li key={"qr__" + idx.toString()}>
                    <a href="#" className={(serialD === qrc[1]) ? "on" : ""} onClick={(e) => qrClick(qrc[1])}>{
                      qrc[1]
                    }</a>
                  </li>
                ))}
