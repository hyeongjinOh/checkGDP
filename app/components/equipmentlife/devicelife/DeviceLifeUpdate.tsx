...
import { useNavigate } from "react-router-dom";
 ...
 /**
 * @brief EHP 설비수명  - 수명인자 수정 컴포넌트, 반응형 동작
 * @param -
 * @returns react components
 */

//component
function DeviceLifeUpdate(props) {
    //trans
    const t = useTrans();
    //recoil
    ...
    const { data: resInfo, error, isLoading, reload, run, } = useAsync({
        ...
        appQuery: { itemIdPk: listItem.itemIdPk },
        ...
        <>
            ...
            <div className="area__right">
                <ul className="page-loca">
                    ...
                    <li>{selTree.room.roomName}</li>
                </ul>
                <h2>{selTree.room.roomName}</h2>
                <div className="inline mb-18">
                    ...
                    <a href="#" className="move-list active" >{t("LABEL.수명인자설정")}</a>
                </div>

                {/* <!--area__right_content, 오른쪽 컨텐츠 영역-->*/}
                {/* <!--221206, scrollH-no 클래스 추가--> */}
                <div className="area__right_content scrollH-no">
                    <div className="tbl__top mb-16 mt-12 d-lm-block">
                        <div className="left">
                            <p className="title">Device  {t("LABEL.정보")}</p>
                        </div>
                    </div>
                    <div className="tbl-list td-h26 h-auto">
                        <table summary="설비 명,Panel 명,모델 명,시리얼 번호,정격전압,Basic,Premium,Advanced 항목으로 구성된 Device 정보 입니다.">
                            ...
                            <thead>
                                <tr>
                                    <th scope="col" className="txt-left"><span>{t("FIELD.기기명")}</span></th>
                                    <th scope="col" className="txt-left"><span>{t("FIELD.Panel명")}</span></th>
                                    <th scope="col" className="txt-left"><span>{t("FIELD.모델명")}</span></th>
                                    <th scope="col" className="txt-left"><span>{t("FIELD.시리얼번호")}</span></th>
                                    <th scope="col" className="txt-left"><span>{t("FIELD.정격전압")}</span></th>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td className="txt-left">{(listItem.basicAssess.totalScore !== null) ? listItem.basicAssess.totalScore : "-"}</td>
                                    <td className="txt-left">{(listItem.premiumAssess.totalScore !== null) ? listItem.premiumAssess.totalScore : "-"}</td>
                                    <td className="txt-left">{(listItem.advancedAssess.totalScore !== null) ? listItem.advancedAssess.totalScore : "-"}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="tbl__top mb-16 mt-32 d-lm-block">
                        <div className="left">
                            <p className="title">{t("LABEL.설정항목")} </p>
                        </div>
                        <div className="right">
                            {(updatelist.filter((cke) => (cke.checked)).length <= 0) ?
                                <button type="button" className="bg-gray" disabled><span>{t("LABEL.저장")}</span></button>
                                :
                                <button type="button" className="bg-blue js-open" data-pop={"pop-change"} onClick={(e) => savedPop(e, updatelist)}>
                                    <span data-pop={"pop-change"}>{t("LABEL.저장")}</span>
                                </button>
                            }
                        </div>
                    </div>
                    <div className="tbl-list type2 td-h26 h530 scrollH">
                        <table summary="선택,No.,점검단계,항목,진단점검 결과 값,사용자 입력 값 항목으로 구성된 설정 항목 목록 입니다.">
                            ...
                            <thead>
                                <tr>
                                    <th scope="col" className="txt-left">{t("FIELD.선택")}</th>
                                    <th scope="col" className="txt-left">{t("FIELD.No")}.</th>
                                    <th scope="col" className="txt-left">{t("FIELD.점검단계")}</th>
                                    <th scope="col" className="txt-left">{t("FIELD.항목")}</th>
                                    <th scope="col" className="txt-left">{t("FIELD.진단점검결과값")} </th>
                                    <th scope="col" className="txt-left">{t("FIELD.사용자입력값")} </th>
                                </tr>
                            </thead>
                            <tbody>
                                {(updatelist) && updatelist.map((list, idx) => (
                                    <tr key={"life_" + idx.toString()}>
                                        ...
                                        {(list.stepName == "User Defined") ?
                                            <td className="txt-left" >
                                                <div className="input__area w85p">
                                                    {(list.checked == true) &&
                                                        <input type="text" placeholder={t("MESSAGE.T1입력하세요", [t("MESSAGE.항목값")])} id={"checkItemName" + list.number}
                                        ...
                                        <td className="txt-left">
                                            <div className="input__area">
                                                {(list.checked == true) &&
                                                    <input type="text" placeholder={t("MESSAGE.T1입력하세요", [t("MESSAGE.설정값")])}
                    ...
                    {/* <!--221206, 문구 추가--> */}
                    <div className="tbl__bottom inline right">
                        <p className="txt-info">* '-' 표시 항목은 진단점검 결과가 없어서 표시하지 않았습니다.</p>
                    </div>
                </div>
                {/* <!--//area__right_content, 오른쪽 컨텐츠 영역-->*/}
            </div>
...
export default DeviceLifeUpdate;
