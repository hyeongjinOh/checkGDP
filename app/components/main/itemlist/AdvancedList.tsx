...
function AdvancedList(props) {
  ...
  // mobile check용 searchinput 영역을 넣는다.
  //검색설정 초기화
  useEffect(() => {
    handleCurPage(0);
    setSearchText("");
  ...
  const { data: data, error, isLoading, reload, run } = useAsync({
    ...
    appPath: "/api/v2/product/company/zone/subzone/room/panel/item/step/advanced?" + appPath,
    appQuery: {
    },
    userToken: userInfo.loginInfo.token,
    //watch: appPath+listMode+stepDone
    watch: appPath + listMode + curItem.assessment.assessmentId + stepDone + isMobile + statusReload
  ...
  useEffect(() => {
    ...
    if (data) {
      if (data.codeNum == 200) {
        clog("IN ADVANCED LIST : " + JSON.stringify(data.body));
  ...
  */
  ...
  function onClickListDetailOpen(e, ehcType, item) {
    clog("IN ADVANCED : onClickListDetailOpen : " + e);
  ...
  function handleCurItemOLD(item) {
    clog("handleCurItem : USERINFO : " + JSON.stringify(userInfo));
  ...
  function handleCurItem(item) {
    clog("handleCurItem : USERINFO : " + JSON.stringify(item));

    if ((item.itemStatus == "NEEDED") && (item.itemStep == "PREMIUM_DONE")) {
      if ((userInfo.loginInfo.role === CONST.USERROLE_ADMIN) || (userInfo.loginInfo.role === CONST.USERROLE_ENGINEER)) {
        //itemCheckRequested(item); 요청완료 단계가 없음
        itemCheckAccepted(item);
      }
    } else if ((item.itemStatus == "REQUESTED") && (item.itemStep == "PREMIUM_DONE")) {
      if ((userInfo.loginInfo.role === CONST.USERROLE_ADMIN) || (userInfo.loginInfo.role === CONST.USERROLE_ENGINEER)) {
        itemCheckAccepted(item);
      }
    } else if ((item.itemStatus == "NEEDED") && (item.itemStep == "ADVANCED_DONE")) {
      alert("진단/컨설팅");
    } else {//if ( (item.itemStatus == "ACCEPTED")&&(item.itemStep == "PREMIUM_DONE") ) {
      itemCheck(item);
    }
  ...
  function itemCheck(item) {
    ...
    if (userInfo.loginInfo.role == CONST.USERROLE_USER) {
      ...
      if (curItem.itemId.length <= 0) { // 현재 아이템이 체크되어 있지 않으면...클릭한 아이템을 현재 아이템으로 함
        clog("IN ADVANCED : " + clickItem.itemId + " : " + "현재 아이템이 체크되어 있지 않으면...클릭한 아이템을 현재 아이템으로 함/단계도 처음으로");
  ...
  // 노후교체 요청 by 20221115-hjo
  async function itemChange(e, item) {

    setParentPopWin("pop-reason",
      <ConsultingPop
        htmlHeader={<h1>진단/컨설팅</h1>}
    ...
    <>
      ...
      <article className={`box list ${(isLoading || (reportLoading)) ? "loading__box" : ""}`}>
        <div className="box__header">
          <p className="box__title"><span className="txt-yellow">Advanced</span> e-HC List</p>
            ...
            <table summary="기기 명, 제조번호, 담당자, 최근점검일, Step, Report, Memo, 성적서, 임시저장, 점검 진행 항목으로 구성된 Basic e-HC List 입니다.">
              ...
              <tbody>
                {itemCheckList.map((item, idx) => (
                  <tr key={item.itemId + "_advanced_" + idx} className={(item.itemId === curItem.itemId) ? "focused" : ""}>
                    {/*clog("IN ADVANCED : ITEM : " + JSON.stringify(item))*/}
                    ...
                    {/* || ((searchInput.clientHeight <= 0) && (searchInput.clientWidth <= 0)))*/
                      /*  (userInfo.role == CONST.USERROLE_USER) ? // by 20221115 - hjo engineer,admin 계정 팝업 변겯 || ((searchInput.clientHeight <= 0) && (searchInput.clientWidth <= 0)))  
                    ...
                    <td className="d-sm-none">
                      ...
                      {(!item.serialNo)
                        ...
                        : <button type="button"
                          ...
                          onClick={(e) => onClickReportView(e, "ADVANCED", item)}>
                          <span className="hide">성적서</span>
                        </button>
                      }
                    ...
                    <td>
                      {/*ADVANCED_ING : 진행중 */}
                      {(item.itemStatus == "NEEDED") && (item.itemStep == "PREMIUM_DONE")
                        ...
                        >
                          <span>{t("LABEL.요청")}</span>{/**Advanced에서는 ROLE_ENGINEER만이  Advanced 진단점검을 요청할수 있음*/}
                        </button>
                        : (item.itemStatus == "REQUESTED") && (item.itemStep == "PREMIUM_DONE")
                          ? <button type="button" className="bg-lightgray"
                            onClick={(e) => handleCurItem(item)}
                          >
                            <span>{t("LABEL.요청완료")}</span>
                          </button>
                          : (item.itemStatus == "ACCEPTED") && (item.itemStep == "PREMIUM_DONE")
                            ? <button type="button" className="bg-middlegray"
                              //onClick={(e)=>(userInfo.loginInfo.role===CONST.USERROLE_ENGINEER)&&itemCheck(item)}
                              onClick={(e) => itemCheck(item)}
                            >
                              <span>{t("LABEL.접수완료")}</span>
                            </button>
                            : (item.itemStatus == "NEEDED") && (item.itemStep == "ADVANCED_DONE")
                              ? <button type="button" className="bg-pink js-open" data-pop={"pop-reason"}
                                onClick={(e) => itemChange(e, item)} // added
                              >
                                <span data-pop={"pop-reason"}>{t("LABEL.진단컨설팅")}</span>
                              </button>
...
function ConsultingPop(props) {
  //recoil
  const userInfo = useRecoilValue(userInfoLoginState);
  //porps
  const item = props.item;
  const setParentStatusReload = props.setParentStatusReload;
  const setStatusReload = props.setStatusReload;
  //교체요청 API
  ...
  async function onClickReCheck(e, item) {
    ...
    if (data) {
      if (data.codeNum === CONST.API_200) {
        alert("Basic에서 점검진행을 해주세요.\n이전 점검 내역은 Report > 진단점검 Report 에서 조회 가능합니다.")
        setStatusReload(true);
        CUTIL.jsclose_Popup("pop-reason");
  ...
  return (
    <>
      <div className="popup__body">
        <p >노후교체 요청 또는 재진행 하시겠습니까?</p>

      </div>
      <div className="popup__footer">
        <button type="button" className="bg-gray" onClick={(e) => onClickReCheck(e, item)} ><span>재진행</span></button>
        <button type="button" className="close" onClick={(e) => Change(item)} ><span>노후교체 요청</span></button>
      </div>
