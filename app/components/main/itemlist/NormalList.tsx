...
function NormalList(props) {
  ...
  const { data: data, error, isLoading, reload, run } = useAsync({
    ...
    watch: appPath + isMobile + pageInfo.listReload + statusReload
              ...
              <thead>
                <tr>
                  ...
                  <th scope="col" className="d-sm-none">
                    <span>{t("FIELD.성적서")}</span>
                  </th>
                  {/*<!--비활성화일경우 클래스에 disabled -->*/}
                  {/*<!--221012, 점검진행 노출-->*/}
                  <th scope="col"><span>{t("FIELD.점검진행")}</span></th>
                </tr>
              </thead>
              <tbody>
